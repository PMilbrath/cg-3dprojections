...
function drawCube(model){
    ...
    model.vertices.push(Vector4(center-(width/2), center+(height/2), center-(depth/2)));  //front top left
    model.vertices.push(Vector4(center+(width/2), center+(height/2), center-(depth/2)));  //front top right
    model.vertices.push(Vector4(center-(width/2), center-(height/2), center-(depth/2)));  //front bottom left
    model.vertices.push(Vector4(center+(width/2), center-(height/2), center-(depth/2)));  //front bottom right
...
function drawCylinder(model){
    ...
    for(let i = 0; i < vertices.length; i++){
        let circleEdges = [i, (i+1) % model.vertices.length];
        if(i%2 == 0){
